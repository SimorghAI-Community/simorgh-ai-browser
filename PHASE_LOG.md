...
هدف اصلی: وقتی یک سشن جدید با Claude (یا هر Agent دیگر) باز می‌شود، کافی است آخرین ورودی این فایل + `ARCHITECTURE.md` داده شود تا Agent بدون خواندن کل تاریخچه commit ها بفهمد:

1. چه چیزی تا الان ساخته شده.
2. چرا به همین شکل ساخته شده (اگر با ARCHITECTURE.md فرقی داشته).
3. فاز بعدی دقیقاً باید از کجا شروع شود.
...
2. **هر فاز باید به اندازه یک session قابل‌مدیریت باشد** — نه یک نسخه کامل (v0.1)، بلکه یک تکه کوچک و قابل تایید از آن (مثلاً «اسکلت Gradle»، نه «کل Browser Core»).
3. **در شروع سشن جدید، فقط آخرین ورودی (یا چند ورودی اخیر مرتبط) را به Agent بدهید**، نه کل فایل را — مگر اینکه واقعاً لازم باشد.
...
5. هر ورودی باید طوری نوشته شود که یک Agent بدون هیچ context دیگری بتواند فقط با خواندن همان یک ورودی، وضعیت را دقیق بفهمد و فاز بعدی را شروع کند.
